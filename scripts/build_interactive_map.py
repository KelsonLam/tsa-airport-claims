...
<section>
  <div class="sect">
    <h2>Top 10 airports</h2>
    <span class="note" id="tabnote"></span>
  </div>
  <div class="card tblwrap"><table id="topTable"></table></div>
</section>
...
document.getElementById('tabnote').textContent = 'By total claim volume, all types';
...
function renderMap() {
  ...
  DATA.airports.forEach(a => {
    ...
    const dot = el('circle', {cx: x(a.lon), cy: y(a.lat), r: r.toFixed(2),
      fill: color, opacity: .58, stroke: '#0B1220', 'stroke-width': 1}, svg);
    ...
    if (a.claims >= DATA.airports[4].claims) {
      const label = el('text', {x: x(a.lon), y: y(a.lat) - r - 5,
...
function renderTable() {
  const top10 = [...DATA.airports].sort((a, b) => countFor(b) - countFor(a)).slice(0, 10);
  document.getElementById('topTable').innerHTML =
    '<tr><th>Code</th><th>Airport</th><th>City</th><th>Claims</th></tr>' +
    top10.map(a => `<tr><td class="mono">${a.code}</td><td>${a.name}</td>` +
      `<td>${a.city}</td><td>${fmt.format(countFor(a))}</td></tr>`).join('');
}
